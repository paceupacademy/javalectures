...
public class JDBCMain {
	 ...
	 * 
	 * Why return Connection? ---------------------- 
	 * - Connection is the "bridge" between Java and the database. 
	 * - Returning it allows other methods (create, read, update, delete) to reuse the same active link instead of reconnecting
	 * manually. 
	 * - Each CRUD method needs a Connection to prepare statements and
	 * execute queries.
	 */
	public static Connection connect() throws SQLException {
		...
		String createTableQuery = "CREATE TABLE IF NOT EXISTS students (" + "id INT PRIMARY KEY, " + "name VARCHAR(100)"
				+ ")";
		...
		return conn; // Return active connection for reuse
	}
	...
	/**
	 * READ operation → Fetch all students. Shows Statement usage, with
	 * PreparedStatement code commented for reference.
	 */
	public static void readStudents() {
		...
		try (Connection conn = connect(); Statement stmt = conn.createStatement()) {
			 ...
			 * PreparedStatement pstmt = conn.prepareStatement(sql); pstmt.setInt(1, 2); //
			 * Example: fetch student with id=2 ResultSet rs = pstmt.executeQuery();
			 */
