 ...
 * Future -  When we submit Callable to ExecutorService it will return future object. get()- waits and return the result of computation , isDone()- check if task is completed, cancel() - Attempts to cancel the execution (your order token lets out collect dish from counter
 ...
 * 
 * 
 * Usually Thread.start -->run Runnable (no return) Simple execution, no return value
 * 
 * Callable  ---> submit() ---> ExcutorService ----> Worker Thread ---> Future result ---> future.get() ---> return value
 * 
 * Callable - Like Runnable, but can return a result
 * ExecutorService - Manages thread(instead of creating them manually)
 * Future - Used to get result of Callable asynchronously
 */
